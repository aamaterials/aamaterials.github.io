...
function init(){
  ...
  axesLabels = ['Void Fraction', 'Metal Density, g/cm\u00B3', 'Density, g/cm\u00B3', 'Pore Limiting Diameter, \u212B', 'Largest Cavity Diameter, \u212B', 'Volumetric Accessible Surface Area, m\u00B2/cm\u00B3',
  'Gravimetric Accesible Surface Area, m\u00B2/g', 'Pore Volume, cm\u00B3/g', 'Qst CO', 'Qst N2', 'Delta Qst','CO desorption','N2 Desorption', 'CO 40 bar', 'N2 40 bar',
  'CO 200K', 'N2 200K', 'CO PTSA', 'N2 PTSA', 'S desorption', 'S PSA', 'S TSA', 'S PTSA', 'Delta PSA', 'Delta TSA', 'Delta PTSA', 'R PSA', 'Void', 'R PTSA', 'Purity PSA',
'Purity TSA', 'Purity PTSA'];

  shortLabels = ['VF', 'Metal d., g/cm\u00B3', 'd, g/cm\u00B3 ', 'PLD, \u212B', 'LCD, \u212B', 'VASA, m\u00B2/cm\u00B3', 'GASA, m\u00B2/g',
                  'PV, cm\u00B3/g', 'Q CO', 'Q N2', 'Delta Q','CO des','N2 des', 'CO 40 bar', 'N2 40 bar',
                  'CO 200K', 'N2 200K', 'CO PTSA', 'N2 PTSA', 'S des', 'S PSA', 'S TSA', 'S PTSA', 'Delta PSA', 'Delta TSA', 'Delta PTSA', 'R PSA', 'Void', 'R PTSA', 'Purity PSA',
                'Purity TSA', 'Purity PTSA'];

  // Get data table from browser cache if possible
  //var dataString = localStorage.getItem('aureliamofs4');
  var db;
  var requestDB = window.indexedDB.open("Nakul");

  requestDB.onupgradeneeded = function(event){
    db = requestDB.result;
    db.createObjectStore("con2", {keyPath: "id"});

    var rangeString = encodeURIComponent('range=A:AG');
    var query = new google.visualization.Query(
      'https://docs.google.com/spreadsheets/d/1l2abdRwwMQ1NPgHCIhLVn4myP4khkaiHzuH8KwvF80A/gviz/tq?gid=0&headers=1&' + rangeString);
  ...
  requestDB.onsuccess = function(event){
    ...
    var request = db.transaction(["con2"]).objectStore("con2").get("01");
...
function handleQueryResponse(response) {
  ...
  requestDB.onsuccess = function(event){
    db = requestDB.result;
    var request = db.transaction(["con2"], "readwrite").objectStore("con2").add({id: "01", data: dataString});
